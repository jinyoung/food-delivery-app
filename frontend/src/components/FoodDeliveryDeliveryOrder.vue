<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-title v-if="value._links">
            배달 주문 # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            배달 주문
        </v-card-title >        
        ...
        <v-card-actions>
            ...
            <v-btn
                ...
                color="primary"
                text
                @click="openAssignDeliveryDriver"
            >
                AssignDeliveryDriver
            </v-btn>
            <v-dialog v-model="assignDeliveryDriverDiagram" width="500">
                <AssignDeliveryDriverCommand
                    @closeDialog="closeAssignDeliveryDriver"
...
<script>
    ...
    export default {
        name: 'FoodDeliveryDeliveryOrder',
        components:{
        },
        props: {
        ...
        data: () => ({
            ...
            assignDeliveryDriverDiagram: false,
        ...
        methods: {
            ...
            async save(){
                try {
                    ...
                    if(!this.offline) {
                        if(this.isNew) {
                            temp = await axios.post(axios.fixUrl('/deliveryOrders'), this.value)
                        } else {
            ...
            async assignDeliveryDriver(params) {
                try {
                    if(!this.offline) {
                    ...
                    this.editMode = false;
                    this.closeAssignDeliveryDriver();
                } catch(e) {
                    this.snackbar.status = true
                    if(e.response && e.response.data.message) {
                        this.snackbar.text = e.response.data.message
            ...
            openAssignDeliveryDriver() {
                this.assignDeliveryDriverDiagram = true;
            },
            closeAssignDeliveryDriver() {
                this.assignDeliveryDriverDiagram = false;
            },
