<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-title v-if="value._links">
            음식 주문 # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            음식 주문
        </v-card-title >        
        ...
        <v-card-actions>
            ...
            <v-btn
                ...
                color="primary"
                text
                @click="openUpdateOrderStatus"
            >
                UpdateOrderStatus
            </v-btn>
            <v-dialog v-model="updateOrderStatusDiagram" width="500">
                <UpdateOrderStatusCommand
                    @closeDialog="closeUpdateOrderStatus"
...
<script>
    ...
    export default {
        ...
        data: () => ({
            ...
            updateOrderStatusDiagram: false,
        }),
	async created() {
        },
        methods: {
            ...
            async save(){
                try {
                    ...
                    if(!this.offline) {
                        if(this.isNew) {
                            temp = await axios.post(axios.fixUrl('/foodOrders'), this.value)
                        } else {
            ...
            openUpdateOrderStatus() {
                this.updateOrderStatusDiagram = true;
            },
            closeUpdateOrderStatus() {
                this.updateOrderStatusDiagram = false;
            },
        },
    }
</script>
