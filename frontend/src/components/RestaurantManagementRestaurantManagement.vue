<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-title v-if="value._links">
            음식 가게 관리 # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            음식 가게 관리
        </v-card-title >        
        ...
        <v-card-text>
            <String v-if="editMode" label="음식점 ID" v-model="value.restaurantId" :editMode="editMode" :inputUI="''"/>
            ...
            <String label="주문 내역" v-model="value.orderHistory" :editMode="editMode" :inputUI="''"/>
        </v-card-text>
...
<script>
    ...
    export default {
        name: 'RestaurantManagementRestaurantManagement',
        components:{
        },
        props: {
        ...
        methods: {
            ...
            async save(){
                try {
                    ...
                    if(!this.offline) {
                        if(this.isNew) {
                            temp = await axios.post(axios.fixUrl('/restaurantManagements'), this.value)
                        } else {
